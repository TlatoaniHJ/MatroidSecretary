...
import org.example.core.buildMatroidSecretaryLPKnownOrderTight
import java.io.File
import kotlin.math.min
import kotlin.system.exitProcess

fun main() {
    testKnownOrderTruncationConjecture()
}

fun testKnownOrderTruncationConjecture() {
    val n = 7
    val log = fileLog(File("matroids_7_known_order.txt"))
        ...
        for (order in permutations(matroid.elements())) {
            if (order !in seenOrders) {
                ...
                for (isTruncation in listOf(false, true)) {
                    ...
                    val timer = Timer()
                    var builder = GurobiLinearProgramBuilder(logToConsole = false, threads = 8)
                    buildMatroidSecretaryLPKnownOrder(0.5, if (isTruncation) truncated else matroid, order, builder)
                    builder.updateModel()
...
fun fileLog(file: File): (Any) -> Unit = { line ->
    println(line)
    file.appendText("$line\n")
}

fun testKnownOrderTightConjecture() {
    val n = 6
    val log = fileLog(File("matroids_6_known_order_tight.txt"))
    val matroids = parseMatroidsFile(File("matroids09_bases"), targetSize = n)
    log("num matroids = ${matroids.size}")
    val withAutomorphisms = matroids.withIndex().map { (index, matroid) -> Pair(index, matroid.automorphisms().size) }
    log("computed automorphisms")
    ...
    for ((index, numAutomorphisms) in withAutomorphisms) {
        ...
        for (order in permutations(matroid.elements())) {
            if (order !in seenOrders) {
                ...
                val ratios = mutableListOf<Double>()
                for (isTight in listOf(false, true)) {
                    log("\t${if (isTight) "TRUNCATION" else "ORIGINAL"}")

                    val timer = Timer()
                    var builder = GurobiLinearProgramBuilder(logToConsole = false, threads = 4)
                    if (isTight) {
                        buildMatroidSecretaryLPKnownOrderTight(0.5, matroid, order, builder)
                    } else {
                        buildMatroidSecretaryLPKnownOrder(0.5, matroid, order, builder)
                    }
                    builder.updateModel()
